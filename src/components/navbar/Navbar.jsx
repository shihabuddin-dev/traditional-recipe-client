    ...
    <nav className="bg-base-200 shadow-sm border-b-1 border-base-300 fixed top-0 left-0 right-0 z-50 w-full">
      <div className="max-w-7xl mx-auto py-2 px-4 md:px-6 lg:px-8 flex justify-between items-center">
        ...
        <ul className="hidden lg:flex gap-5 font-semibold text-[#1a1a1a]">
          ...
          <li>
            <NavLink to="/gallery" className={linksClass}>
              <FaImages />
              Gallery
            </NavLink>
          </li>
        ...
        <div className="hidden space-x-2 lg:flex items-center">
          {user ? (
            ...
            >
              ...
              />
              <div
                className={`absolute right-0 mt-2 w-40 bg-base-300 rounded-2xl shadow-lg transition-opacity duration-200 ${showDropdown ? "opacity-100" : "opacity-0 pointer-events-none"
                  }`}
              >

                <Link to="/dashboard/my-profile">
                  <p className="hover:underline px-4 text-sm font-medium text-orange-600 py-2">
                    My Profile
                  </p>
                </Link>
                <button
